...
def _print_result(output, result):
    ...
    for row in result['rows']:
        output.print_md(u"- **{0}** | Ag:{1} | Puan:{2}/5 | Katki:{3} | {4}".format(
            row.get('category', u''), row.get('weight', u''), row.get('score5', u''), row.get('weighted_score', u''), row.get('note', u'')))
...
def main():
    ...
    result['summary'] = {
        ...
        'category_scores': dict((row.get('category', u''), row.get('score5', 0)) for row in result.get('rows', [])),
        'metrics_snapshot': {},
